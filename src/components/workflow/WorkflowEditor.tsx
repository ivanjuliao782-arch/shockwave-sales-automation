...
const WorkflowNode = ({ id, type, title, subtitle, x, y, icon, color, highlight }: NodeProps) => {
...
className={cn(
"absolute p-4 rounded-xl border-2 transition-all cursor-pointer select-none",
...
// Resources connected to the AI agent
...
{/* Node Library Sidebar (n8n Style) */}
<motion.div
initial={false}
animate={{
...
{/* Toggle Button for closed state - only if we want an internal trigger too */}
{!isSidebarOpen && (
<motion.button
...
{/* Shockwave Trademark Headline - Dynamic visibility on scroll */}
<motion.div
...
{/* Connections */}
<Connection start={[540, 240]} end={[630, 240]} color="#4a4d6d" />
<Connection start={[870, 240]} end={[960, 240]} color="#4a4d6d" />
<Connection start={[1200, 240]} end={[1290, 140]} color="#4a4d6d" />
<Connection start={[1200, 240]} end={[1290, 340]} color="#4a4d6d" />
{/* Resource Connections */}
<Connection start={[650, 450]} end={[690, 280]} color="#10b981" dashed />
<Connection start={[850, 450]} end={[750, 280]} color="#10b981" dashed />
<Connection start={[570, 600]} end={[630, 280]} color="#06b6d4" dashed />
{/* Nodes */}
{nodes.map(node => (
